...
export const creatorAPI = {
    ...
    getCallgirl: (params?: { page?: number; limit?: number; province?: string; district?: string; minPrice?: number; maxPrice?: number; sortBy?: string }) => {
        const qp: Record<string, string> = {}
        if (params?.page) qp.page = String(params.page)
        if (params?.limit) qp.limit = String(params.limit)
        if (params?.province) qp.province = params.province
        if (params?.district) qp.district = params.district
    ...
    getRelatedCreator: (id: number, limit: number = 10) => api.get(`/creators/related/${id}`, { limit: String(limit) }),
